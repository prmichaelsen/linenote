...
export const cleanUpOrphanedNotes = async () => {
  ...
  if (!["on-interval", "on-save-and-on-interval"].some((s) => s === strategy)) {
    return;
  }
  const globalNoteCache = getNoteCache();
  const files = await getIncludedFilePaths();
  const uuids = (
    await Promise.all(files.map((file) => getUuidFromNotePath(file)))
  ).filter(isTruthy);
  const activeUuids = globalNoteCache
    .keys()
...
export const getOrphanedUuidsForCurDoc = log.time(
  ...
  (props: GetOrphanedNoteMarkersProps): string[] => {
    const globalNoteCache = getNoteCache();
